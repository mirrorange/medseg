...
- 批量运行为 **每个子集创建一个独立任务**
- 样本集级别提供 **首选操作按钮**（对应 `primary` 模块）：相当于选中首选模块的所有建议子集批量处理
- 运行 Pipeline 时 **直接使用选中的子集** 作为输入，无需在对话框中手动选择
...
| 样本集详情页 | 文件浏览器风格：子集列表（根级）/ 图像列表（子集内）、列表/网格双视图、右键菜单含管线操作建议、首选操作按钮、属性对话框、子集内上传图像 |
