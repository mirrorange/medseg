...
│   │   ├── library.tsx                 # /app/library/* — 样本库（路径驱动）
...
│   │   │   ├── library-path.ts         # 路径片段与 Library URL 的纯函数工具
...
│   │   │   ├── use-library-route-navigation.ts # 基于 route 的导航 helper（Back/Forward/Up/路径跳转）
...
export default [
  ...
  layout("routes/layout.tsx", [
    ...
    // 样本库使用路径段表示当前目录，例如 /app/library/folder-a/subfolder-b
...
| 登录 | `/login` | 用户名 + 密码登录 |
| 注册 | `/register` | 注册新用户 |
| 样本库 | `/app/library/*` | 类文件管理器浏览、列表/网格视图切换、路径导航、拖拽移动、右键菜单、创建/重命名/移动/删除文件夹与样本集。**当前目录使用路径段写入 URL**（如 `/app/library/folder-a/subfolder-b`），支持书签、刷新和直接分享链接 |
| 共享样本库 | `/app/shared` | 浏览公共共享的样本集、复制到个人库 |
| 样本集详情 | `/app/sample-sets/:id` | **面包屑导航**（Library / [文件夹] / 样本集名称）+ 顶部信息区（名称/描述/首选操作/分享）+ 文件浏览器风格：列表/网格双视图、子集列表（根级）→ 图像列表（子集内）、右键菜单含管线操作建议和预览入口、属性对话框。描述支持多行文本，使用 textarea 编辑 |
...
- 右键上下文菜单（右键自动选中目标项）
- 名称唯一性约束与友好错误提示
- 路径即状态：Library URL 使用可读路径段表达当前目录，route 是导航真源
...
export interface LibraryItem {
  ...
  child_count?: number | null;   // 仅 folder
  created_at: string;
  updated_at: string;
}
...
**数据流**：Library route 先根据 URL 路径片段调用 `GET /api/library/resolve` 解析目标目录，再由 `navigateTo(folderId)` 调用 `GET /api/library/contents?folder_id=...` 更新 `items`、`breadcrumb`、`folderId`。

**职责边界**：
- Route / URL 是导航真源，负责表达“当前在哪个目录”
- `LibraryStore` 只负责“当前目录内容、选择状态、视图状态”的加载与缓存，不再维护独立的导航历史
- `use-library-route-navigation.ts` 统一封装 `Back / Forward / Up / goToChild / goToBreadcrumb` 等路径跳转行为
...
- 当 route loader 解析出的 `initialFolderId` 变化时触发 `navigateTo(initialFolderId)`，但组件本身不再把 store 状态反向写回 URL，避免形成 route/store 导航环
...
- 导航按钮：后退 (`←`)、前进 (`→`)、上级 (`↑`)，统一通过 `useLibraryRouteNavigation()` 调用 React Router 导航
...
- **编辑态**：Input 输入框显示完整路径文本（`Library/Folder A/...`），自动聚焦并全选，按 Enter 或失焦后将输入解析为路径段并导航到对应 route，按 Escape 取消
- 根级始终显示 "Library"
...
- 每个卡片：类型图标（大号） + 名称（居中截断） + 描述（`line-clamp-2` 截断，支持多行显示）
...
├── 面包屑导航（Library / [文件夹名] / 样本集名称）
├── 样本集名称 / 描述（inline edit，描述使用 textarea 支持多行）
...
```
┌─────────────────────────────────────────────────────────────┐
│  Library / CT Scans / 📄 样本集名称        ← 面包屑导航      │
│  描述文字（可点击编辑，支持多行 textarea）     [▶ 首选操作] [分享]   │
└─────────────────────────────────────────────────────────────┘
```

- **面包屑导航**：展示路径 `Library / [folder_name] / sample_set_name`
  - "Library" 链接到 `/app/library`
  - 文件夹名链接到对应的 path route（如 `/app/library/folder-a/subfolder-b`）
  - 链接构造依赖后端 `SampleSetDetail` 返回的 `folder_id` + `folder_name`，并额外调用 `GET /api/library/path/{folder_id}` 获取完整祖先链
- **样本集名称 / 描述**：直接展示，点击进入编辑模式（inline edit），失焦或 Enter 保存
  - 描述使用 `<textarea>` 替代 `<input>` 以支持多行文本
  - 非编辑状态下，描述以 `whitespace-pre-line` 样式展示多行内容
...
Library 路由页面 (`routes/library.tsx`) 负责把 path route 解析为内部 `folderId`：
...
export async function clientLoader({ params }: Route.ClientLoaderArgs) {
  ...
  const resolved = await resolveLibraryPath(pathSegments);
  return { initialFolderId: resolved.folder_id };
}
...
`clientLoader` 不再预加载目录内容本身，但会先把 URL 中的路径片段解析成目标 `folderId`。实际目录内容仍由 `LibraryStore.navigateTo()` 加载。
