...
| 登录 | `/login` | 用户名 + 密码登录 |
| 注册 | `/register` | 注册新用户 |
| 样本库 | `/app/library` | 类文件管理器浏览、列表/网格视图切换、路径导航、拖拽移动、右键菜单、创建/重命名/移动/删除文件夹与样本集。**当前文件夹 ID 同步到 URL 查询参数**（`?folder=<uuid>`），支持书签、刷新和直接分享链接 |
...
- **面包屑导航**：展示路径 `Library / [folder_name] / sample_set_name`
  ...
  - 文件夹名链接到 `/app/library?folder=<folder_id>`（仅当 folder_id 存在时显示）
  - 面包屑数据依赖后端 `SampleSetDetail` 返回的 `folder_id` + `folder_name` 字段
