...
| 样本集详情 | `/app/sample-sets/:id` | 文件浏览器风格：列表/网格双视图、子集列表（根级）→ 图像列表（子集内）、右键菜单含管线操作建议、首选操作按钮、属性对话框 |
...
样本集详情页采用文件浏览器风格，分为两层浏览：

**根级（子集列表）**：
1. **浏览子集**：显示所有子集，类似目录列表
2. **进入子集**：双击子集进入内部图像列表
3. **新建子集**：工具栏或右键空白处新建
4. **重命名子集**：右键子集 → 重命名（F2 快捷键）
5. **删除子集**：右键子集 → 删除（Delete 快捷键）
6. **查看属性**：右键子集 → 属性，打开属性对话框（查看/编辑详细信息和元数据）
7. **管线操作（右键菜单）**：
   - 右键单个子集：显示该子集的建议处理操作（recommended 模块）
   - 「更多操作」子菜单：显示所有可用的处理模块（available 模块）
   - 选中多个子集时：若多个子集拥有相同的建议模块，允许批量运行
   - 点击管线操作 → 打开运行对话框（输入子集已预填选中项，无需手动选择）
8. **首选操作按钮**：工具栏中的操作按钮，对应 awareness 响应中的 `primary` 模块
   - 点击后：选中首选模块的所有 `recommended_subset_ids` → 批量提交（每子集一个任务）
...
1. **浏览图像**：显示子集内的所有图像
2. **返回上级**：工具栏 ← 按钮返回子集列表
3. **预览图像**：双击图像进入图像查看器 (`/app/viewer/:setId/:subsetId`)
4. **上传图像**：工具栏或右键空白处 → 上传（multipart 上传到当前子集）
5. **重命名图像**：右键图像 → 重命名
6. **删除图像**：右键图像 → 删除
7. **查看属性**：右键图像 → 属性，打开属性对话框（查看格式、元数据等）

> **变更说明**：
> - 移除了原有的样本集级别图像上传（不再自动创建 raw 子集），改为在子集内上传
> - 移除了原有的管线感知建议面板（Pipeline Awareness cards），改为右键菜单中的管线操作
> - 新增首选操作按钮，提供样本集级别的一键批量处理入口
...
│   ├── 刷新按钮
│   └── 首选操作按钮 (▶ Run {primary module}，仅在 subsets 层级且有 primary 时显示)
├── useLassoSelection (use-lasso-selection.tsx)      ← 鼠标框选
│   └── LassoOverlay (选框覆盖层)
...
type SampleSetAction =
  ...
  | "run-pipeline"      // 运行指定管线模块（来自右键菜单）
  | "run-primary"       // 运行首选操作（样本集级别批量）
  | "refresh"           // 刷新
  | "select-all";       // 全选
```
...
**subsets 层级**：`[样本集标题] | [列表|网格] | [新建子集] | [刷新] | [▶ Run {primary module}]`
**images 层级**：`[← 返回] [子集名称] | [列表|网格] | [上传图像] | [刷新]`

- 首选操作按钮仅在 subsets 层级且 `awareness.primary` 存在时显示
- 首选操作按钮标签显示 primary 模块名称，hover 提示显示建议子集数量
- 点击首选操作按钮 → 选中所有 `primary.recommended_subset_ids` → 打开 RunPipelineDialog（预填模块和子集）
...
- **单/批量模式**：
  - 单个子集：显示输出子集名称输入框（文本输入）
  - 多个子集：显示输出名称模板输入框 + 模板预览列表
- **提交**：
  - 单个子集 → 调用 `POST /api/pipelines/run`
  - 多个子集 → 调用 `POST /api/pipelines/batch-run`
...
export async function clientLoader({ params }: Route.ClientLoaderArgs) {
  // 仅加载样本集基本信息（名称、描述），子集等数据由 Store 自行管理
  const { data, error } = await getSampleSet({ path: { id: params.id } });
