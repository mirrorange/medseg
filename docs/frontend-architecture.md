...
1. **进入目录**：双击文件夹进入，地址栏实时更新路径
2. **返回上级**：点击工具栏 ↑ 按钮或地址栏中的上级目录
3. **打开样本集**：双击样本集跳转到样本集详情页
4. **多选操作**：Ctrl/Cmd+点击多选，Shift+点击范围选择，在空白区域拖动鼠标框选
5. **拖拽移动**：选中项拖拽到文件夹上松开完成移动
6. **右键菜单**：右键文件夹/样本集自动选中并弹出操作菜单，右键空白处弹出新建菜单
7. **视图切换**：工具栏按钮切换列表视图（表格，含名称、类型、日期等列）和网格视图（卡片，含图标和名称）
8. **地址栏**：点击路径各段导航到对应目录，点击空白区域进入编辑模式可查看/复制完整路径
